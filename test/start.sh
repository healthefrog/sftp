...
mkdir -p target
docker run -d --name sftp --link sftptest:sftptest -p $SSH_PORT:22 --env-file=sftp/env -v $(pwd)/secrets:/etc/secrets:ro -v $(pwd)/ssl:/etc/ssl:ro -v $(pwd)/target:/target sftp
